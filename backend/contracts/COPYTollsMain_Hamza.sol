...
contract TollsMain {
    ...
    uint256 public constant LAND_PRICE_DEFAULT = 1; // Default price in MATIC

    // Events
    event TollPayment(address indexed user, uint256 latitude, uint256 longitude, uint256 toll_amount);
    ...
    function updateLandAreaPrice(uint256 latitude, uint256 longitude, uint256 newPrice) external {
        ...
        require(landAreas[gridLatitude][gridLongitude].owner == msg.sender, "You are not permitted to change the price");
    ...
    function createPayment(address user, address owner, uint256 paymentAmount, uint256 latitude, uint256 longitude) internal {
        require(address(this).balance >= paymentAmount, "Insufficient balance for payment");
